 ...
 *  TSLA/AMZN carry a lime mark: the assets CorporaX makes dividend-ready. */
    ...
    <div className="flex shrink-0 items-center">
      {UNIVERSE.map((t) => (
        <span key={t.sym} className="flex items-center gap-2 whitespace-nowrap px-5 py-2">
          <span
            className={`h-1.5 w-1.5 rounded-full ${t.enabled ? "bg-lime-bright" : "bg-ink-faint/40"}`}
            aria-hidden
          />
...
export function Ticker() {
  return (
    <div className="border-b border-line bg-surface-raised/60">
