...
> **Requires:** ERC-20 · **Origin:** Parvalon (Robinhood Chain / Arbitrum)
>
...
For convenience, a CAE-1 publisher SHOULD expose a machine-readable HTTP feed that flattens the event-derived state. Parvalon serves this at `GET /api/actions`:
...
3. **Reference implementation** — the Parvalon `CorporateActionRegistry` + `DividendDistributor` as the canonical reference, with the `IActionSource` provenance seam as an optional, pluggable extension.
